...
# Dataset: dat73_1                                                             #
# Date: 01.27.2014                                                             #
# Author: Cheng-Ying Yang <ania.c.yang@gmail.com>                              # 
################################################################################
...
model5<-function(x1, x2, myk) {
        ...
        x1=x1
        obs<-dim(x1)[1]
        nvar<-ncol(x1)  # the number of equations in the system
        nbig<-nvar*nlag  #adjusted by lags
        sample<-obs-nlag+1
        v<-matrix(0,(obs-nlag),nbig)
        for (i in 1:nlag){
             for (n in 1:nvar){ 
                 v[,((i-1)*nvar+n)]<-x1[(nlag-i+1):(obs-i),n]
             }
        }   
	  ...
	  e3<-c(0,0,1,rep(0, nbig-3))
        ...
        iR<-as.vector(e3%*%(t(Xbig))) #not demeaned i 
	  ...
	  edqt<-as.vector(e1%*%((Abig-Ibig)%*%mtx.exp(Abig,myk-1)%*%t(Xtbig)))
        elam<-edqt-ert
	  ert.lm<-lm(ert~iR)   # regression of E(r_t+j-1) on i_t
        ert.lm.nw<-matrix(coeftest(ert.lm,df=Inf,vcov=NeweyWest(ert.lm)),2,2)
	  edqt.lm<-lm(edqt~iR) # regression of E(d(q_t+j)) on i_t
        edqt.lm.nw<-matrix(coeftest(edqt.lm,df=Inf,vcov=NeweyWest(edqt.lm)),2,2)
	  elam.lm<-lm(elam~iR) # regression of E(l_t+j-1) on i_t
        elam.lm.nw<-matrix(coeftest(elam.lm,df=Inf,vcov=NeweyWest(elam.lm)),2,2)
...
for (cno in 1:7){
     ...
     for (k in 1:121){
          reg<-model5(xnew,X2,k)
          coef.ci.blj<-c(-reg[[1]][2,1],-reg[[1]][2,1]-1.645*reg[[1]][2,2],-reg[[1]][2,1]+1.645*reg[[1]][2,2])
          # coef.ci.brj<-c(-reg[[2]][2,1],-reg[[2]][2,1]-1.645*reg[[2]][2,2],-reg[[2]][2,1]+1.645*reg[[2]][2,2])
          # coef.ci.bqj<-c(-reg[[3]][2,1],-reg[[3]][2,1]-1.645*reg[[3]][2,2],-reg[[3]][2,1]+1.645*reg[[3]][2,2])
          coef.ci<-c(coef.ci.blj)    
          mycoef<-rbind(mycoef,coef.ci)
     }
     colnames(mycoef)<-c("blj","blj.l90","blj.u90")

     write.csv(mycoef,file=paste("fig2",mylist[cno],"csv",sep="."),row.names=F)    
}
